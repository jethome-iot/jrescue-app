...
class APHandler:
    ...
    def _wifi_iface(self) -> Optional[str]:
        ...
        # Fallback via nmcli (lists unmanaged devices too).
    ...
    def _gateway_reachable(self, timeout: float = 15.0) -> bool:
        """Success criterion: a default gateway exists AND answers ping."""
        deadline = time.time() + timeout
        while time.time() < deadline:
            gateway = self._default_gateway()
            if gateway:
                ret, _, _ = run_command(
                    ['ping', '-c', '2', '-W', '3', gateway], check=False)
                if ret == 0:
                    return True
            time.sleep(2)
        return False
    ...
    def provision(self, ssid: str, password: str = None,
        ...
        try:
            ...
            # 3. VERIFYING — real connectivity is an IP AND a reachable gateway.
            emit(PHASE_VERIFYING, "Verifying connection")
            ip = self._current_ip(iface) if iface else None
            if not ip:
                time.sleep(2)  # give DHCP a beat
                ip = self._current_ip(iface) if iface else None
            if not ip or not self._gateway_reachable():
                return self._rollback(
                    ssid, "joined but no route to the network (gateway unreachable)", emit)
