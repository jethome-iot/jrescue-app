...
class CaptivePortalHandler(RescueWebHandler):
    ...
    Only the provisioning endpoints are exposed here — the destructive parts of
    the API (flash, reboot, USB) are NOT reachable from an AP client, even though
    the same handler class also serves the full UI on :8124.
    """

    # API the captive portal is allowed to reach (everything else -> 403).
    ALLOWED_GET = (
        '/api/provision/networks',
        '/api/provision/status',
        '/api/network/ap/status',
    )
    ALLOWED_POST = ('/api/provision/connect',)
    ...
    def do_GET(self) -> None:
        ...
        if path.startswith('/api/'):
            if path in self.ALLOWED_GET:
                self.handle_api_get(path)
            else:
                self.send_error_response('Not available on the setup portal', 403)
            return
...
def main() -> None:
    ...
    try:
        # Threaded so the portal's concurrent polls and a blocking handler
        # (e.g. scan_wifi's rescan+sleep) don't serialize/freeze other requests.
        with http.server.ThreadingHTTPServer((host, port), RescueWebHandler) as httpd:
            if verbose:
